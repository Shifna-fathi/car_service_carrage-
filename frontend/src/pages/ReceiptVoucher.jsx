...
const accountHeads = ["Service Income", "Other Income"];
...
export default function ReceiptVoucher() {
  ...
  const getNextVoucherNo = () => {
    if (vouchers.length === 0) return 'RCPT-0001';
    const last = vouchers[vouchers.length - 1].voucherNo;
    const num = parseInt(last.replace('RCPT-', '')) + 1;
    return `RCPT-${num.toString().padStart(4, '0')}`;
  };

  const handleChange = e => setForm({ ...form, [e.target.name]: e.target.value });

  const handleSave = () => {
    if (!form.date || !form.payer || !form.paymentMethod || !form.accountHead || !form.amount) return;
    let updated;
    if (editId) {
    ...
    setVouchers(updated);
    setForm({ date: new Date().toISOString().slice(0, 10), payer: '', description: '', paymentMethod: '', accountHead: '', amount: '', invoice: '' });
    setEditId(null);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto bg-transparent rounded-xl shadow-md p-6 mt-6">
      <h2 className="text-2xl font-bold text-blue-700 mb-4">Receipt Voucher</h2>
      <div className="mb-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
          ...
          <input name="payer" value={form.payer} onChange={handleChange} className="border rounded-lg px-4 py-2" placeholder="Payer Name" />
          ...
          <input name="invoice" value={form.invoice} onChange={handleChange} className="border rounded-lg px-4 py-2" placeholder="Invoice No (optional)" />
        </div>
      ...
      <table className="min-w-full text-sm border border-gray-300">
        <thead className="bg-gray-100">
          <tr>
            ...
            <th className="py-2 px-3 border text-left">Payer</th>
            ...
            <th className="py-2 px-3 border text-left">Invoice</th>
        ...
        <tbody>
          {vouchers.length === 0 ? (
            <tr>
              <td colSpan="9" className="py-6 text-gray-500 text-center">No receipt vouchers yet.</td>
            </tr>
          ) : (
            vouchers.map(v => (
              <tr key={v.id} className="hover:bg-gray-50">
                ...
                <td className="py-2 px-3 border text-left">{v.payer}</td>
                ...
                <td className="py-2 px-3 border text-left">{v.invoice}</td>
